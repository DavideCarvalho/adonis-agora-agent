...
A request that carried a token which was refused adds `error="invalid_token"`; a `403` from `McpAuthError` adds `error="insufficient_scope"`. Without OAuth metadata (`apiKeyAuth()` alone) the challenge is a bare `Bearer`.
