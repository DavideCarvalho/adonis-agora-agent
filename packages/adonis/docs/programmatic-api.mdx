...
A *delegated* sub-agent is handled for you, on **both** runners: a nested run has no human to ask, so its `action` tools — and its question sets — are settled by the runner rather than left hanging.
</Callout>
