...
## Delegated runs have no human

A sub-agent runs behind another agent's tool call. Nobody is watching it: its run id is not the stream a person subscribed to, and the frames it writes arrive in the *parent's* stream, under a turn the reader thinks belongs to the agent they asked. So a delegated run never suspends on a human — on **either** runner:

- an `action` tool is **declined**, with `a delegated sub-agent has no human to ask` as its recorded reason;
- a question set (an `intake` or the model's `ask`) reads that decline as a **skip**, and the sub-agent proceeds on the pre-picked `defaults` its own request carries.

The hook set is one function, `delegatedRunHooks()`, installed by the inline runner's nested loop and by a child `AgentRunWorkflow` alike — and it deliberately supplies **no** `awaitAnswers`, so the two kinds of wait cannot settle differently. Keep the tools that need a human on the agent a human is talking to (`tools:` on the sub-agent's definition).

A child that suspended on a HITL signal under an older build keeps replaying against the history it holds: the change is gated on `ctx.patched('agent:delegated-runs-settle-hitl')`, asked for only on a child, so a top-level run's checkpoint sequence is byte-identical either way.
