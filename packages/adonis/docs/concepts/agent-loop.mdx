...
| `awaitAnswers(request, ctx)` | resolves a pending in-memory promise, on the same key | awaits the same durable signal an approval does |
...
Delegation runs a **nested** in-process loop on a transient sub-thread. A nested sub-agent has no human to prompt, so its human-in-the-loop waits are settled by the runner rather than left hanging — see [Delegated runs have no human](/docs/agent/durability/durable-runner#delegated-runs-have-no-human), which is the same on the durable runner.
