...
| `transientRetry` | In-place retry for a call that failed transiently. Defaults to `{ attempts: 2, backoffMs: 150 }`; `false` surfaces the first failure. Which failures qualify depends on the tool's `kind` — see [An approved action is never issued twice](#an-approved-action-is-never-issued-twice). |
