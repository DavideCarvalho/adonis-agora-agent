 ...
 * mirrors the agent SPA so the refusal reads as part of the same product, not a server default.
 *
...
export interface AccessDeniedInfo {
  ...
  /** The console's mount path (`/agent/dashboard`; `''` when mounted at the root). */
  basePath: string;
...
export interface AccessDeniedPageOptions {
  /** Console name in the eyebrow and `<title>`. Default: the console's own name (`Agent`). */
  brand?: string;
...
export const CONSOLE = {
  brand: 'Agent',
  accent: '#a78bfa',
  packageName: '@adonis-agora/agent',
} as const;
